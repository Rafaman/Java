...
public class PannelloGrafico{
    ...
    public static void main(String[] args) {
        ...
        p.setEtichettaBottone(1, "Trasposta");
        p.setEtichettaBottone(2, "Somma Croce");
        p.setEtichettaBottone(3, "Pulisci");
        p.setEtichettaBottone(4, "Ordina");
    ...
    private void inizializza() {
        ...
        for (int i = 0; i < numeroBottoni; i++) {
            ...
            bottone.setBounds(240, 300 + 30 * i, 150, 30);
            bottoni[i] = bottone;
    ...
    private void trasposta() {
        bloccaMatriceB();
        int[][] mat = convertiInMatriceIntera(getMatriceA());
        int[][] mat2 = convertiInMatriceIntera(getMatriceB());

        for (int i = 0; i < numeroRigheA; i++)
            for (int j = 0; j < numeroColonneA; j++)
                mat2[j][i] = mat[i][j];
        setMatriceB(convertiDaMatriceIntera(mat2));
    }

    private void pulisci() {
        bloccaMatriceB();
        String[][] m = getMatriceA();
        for (int i = 0; i < numeroRigheA; i++)
            for (int j = 0; j < numeroColonneA; j++)
                m[i][j] = "";
        setMatriceA(m);
    ...
    private void sommaCroce() {
        int[][] m = convertiInMatriceIntera(getMatriceA());
        int[][] m2 = convertiInMatriceIntera(getMatriceB());
        for (int i = 0; i < numeroRigheA; i++)
            for (int j = 0; j < numeroColonneA; j++) {
                if (i == 0 && j == 0) {
                    m2[i][j] = m[i][j + 1] + m[i + 1][j];
                }
                if (i == 0 && j == numeroColonneA - 1) {
                    m2[i][j] = m[i][j - 1] + m[i + 1][j];
                }
                if (i == (numeroRigheA - 1) && j == 0) {
                    m2[i][j] = m[i - 1][j] + m[i][j + 1];
                }
                if (i == numeroRigheA - 1 && j == numeroColonneA - 1) {
                    m2[i][j] = m[i - 1][j] + m[i][j - 1];
                }
                if (i == 0 && j < (numeroColonneA - 1) && j > 0) {
                    m2[i][j] = m[i][j + 1] + m[i + 1][j] + m[i][j - 1];
                }
                if (i == numeroRigheA - 1 && j < numeroColonneA - 1 && j > 0) {
                    m2[i][j] = m[i][j + 1] + m[i - 1][j] + m[i][j - 1];
                }
                if (j == 0 && i < (numeroRigheA - 1) && i > 0) {
                    m2[i][j] = m[i - 1][j] + m[i + 1][j] + m[i][j + 1];
                }
                if (j == numeroColonneA - 1 && i < numeroRigheA - 1 && i > 0) {
                    m2[i][j] = m[i - 1][j] + m[i + 1][j] + m[i][j - 1];
                }
                if (i > 0 && i < (numeroRigheA - 1) && j > 0 && j < (numeroColonneA - 1)) {
                    m2[i][j] = m[i - 1][j] + m[i + 1][j] + m[i][j + 1] + m[i][j - 1];
                }
            }
        setMatriceB(convertiDaMatriceIntera(m2));
    }

    private void Ordinata() {
        ...
        for (int i = 0; i < numeroRigheB; i++)
            for (int j = 0; j < numeroColonneB; j++)
                m2[i][j] = elementi.get(j + numeroRigheB * i);
        setMatriceB(convertiDaMatriceIntera(m2));
    }
}
